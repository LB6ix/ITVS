...
describe('Delete /api/hardware/id', (done) => {
  before((done) => {
    conn
      .connectDB()
      .then(() => done())
...
it('administratorius turėtų trinti aparatinę įrangą pagal ID', (done) => {
  request(app)
    .del('/api/hardware/60aaec8ba924dd21a8b61a2a')
    ...
    .then((res) => {
      const body = res.body;
      expect(body).to.include({ msg: 'Apratinės įrangos įrašas ištrintas' });
      done();
    })
    .catch((err) => done(err));
});
